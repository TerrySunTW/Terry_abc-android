...
public class Photo_ImageAdapter extends BaseAdapter {
	...
	public Photo_ImageAdapter(Context c) {
		mContext = c;
	}
	...
	private void zoomImageFromThumb(View thumbView, int position) {
		...
		viewPager = (Photo_HackyViewPager)((Activity)mContext).findViewById(R.id.expanded_image);
		...
		ObjectAnimator alphaAnimator = ObjectAnimator.ofFloat(((Photo_GalleryActivity)mContext).gridView, "alpha", 1.0f, 0.f);
	...
	public boolean getScaleFinalBounds(int position) {
		GridView gridView = ((Photo_GalleryActivity)mContext).gridView;
			...
			photoView
					.setOnPhotoTapListener(new PhotoViewAttacher.OnPhotoTapListener() {
						public void onPhotoTap(View paramAnonymousView,
							...
							ObjectAnimator containAlphaAnimator = ObjectAnimator.ofFloat(((Photo_GalleryActivity)mContext).gridView, "alpha", 0.f, 1.0f);
							if (scaleResult) {
